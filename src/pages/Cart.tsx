    ...
    return (
      <div className="relative min-h-screen flex items-center justify-center px-6 overflow-hidden">
        ...
        <motion.div
          ...
          className="glass p-20 md:p-32 rounded-[60px] border-white/5 text-center relative z-10 max-w-4xl w-full"
        >
          ...
          <p className="text-white/30 mb-16 max-w-sm mx-auto text-lg font-medium">
  ...
  return (
    <div className="relative min-h-screen pb-32 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-32 relative z-10">
        <div className="mb-24">
          <div className="micro-label mb-8 flex items-center gap-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          <div className="lg:col-span-8 space-y-8">
            {cart.map((item, i) => (
              <motion.div
                ...
                className="premium-card p-8 flex flex-col sm:flex-row items-center gap-10 group"
              >
                <div className="w-32 h-32 rounded-3xl overflow-hidden shrink-0 border border-white/10 glass p-1">
                  <img src={item.thumbnailUrl} alt={item.title} className="w-full h-full object-cover rounded-[20px] opacity-80 group-hover:opacity-100 transition-opacity" referrerPolicy="no-referrer" />
                </div>
                <div className="flex-grow text-center sm:text-left">
                  <p className="micro-label text-neon-blue mb-2">{item.category}</p>
                  <h3 className="text-2xl font-bold uppercase tracking-tight mb-2 group-hover:text-neon-blue transition-colors">{item.title}</h3>
                  <p className="text-3xl font-black tracking-tighter">${item.price}</p>
                </div>
                <button 
                  onClick={() => removeFromCart(item.id)}
                  className="p-6 rounded-3xl glass hover:bg-red-500/10 text-white/10 hover:text-red-400 transition-all group/del"
                >
          ...
          <div className="lg:col-span-4">
            <div className="glass p-12 rounded-[60px] border-white/10 sticky top-32 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)]">
              <h3 className="text-2xl font-display uppercase tracking-tighter mb-12">ORDER SUMMARY</h3>
              ...
              <div className="space-y-6 mb-12">
                ...
                <div className="flex justify-between items-end">
                  <span className="micro-label mb-1">Total</span>
                  <span className="text-5xl font-black tracking-tighter neon-text">${total.toFixed(2)}</span>
                </div>
              </div>
